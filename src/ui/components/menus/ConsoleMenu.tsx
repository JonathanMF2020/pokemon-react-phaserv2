...
import { BagItems, BagItemsNames } from "../Bag";
...
export const ConsoleMenu = ({ setSelectedOption }: SettingsMenuProps) => {
  ...
  const handleSubmit = (e) => {
    userDataStore.setObjectToBag(value, 1);
  };
      ...
      <div className="content">
        {(hovered === Options.GENERAL ||
          ...
          <>
            ...
            >
              <label>Set Bag Item</label>
              <select
                value={value}

                onChange={(e) => setValue(Number(e.target.value))}
              >
                <option value={0} disabled={true}>SELECCIONA UNA OPCION</option>
                {BagItemsNames.map((n) => {
                    return (<option value={n.value}>{n.label}</option>);
                })}
              </select>
              <button type="button" color="#ff0000" onClick={handleSubmit}>ADD</button>
            </div>
          </>
        )}
      </div>
